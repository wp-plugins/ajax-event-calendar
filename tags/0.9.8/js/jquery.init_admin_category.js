...
$jq().ready(function(){
		...
		$jq.post(ajaxurl, { action: 'add_category', 'category_data': $jq('#aec-category-form').serialize() }, function(data){
			if (data) {
				var row =  '<li id="id_' + data.id + '"> \n';
					...
					row += '<span><button class="update button-secondary">' + custom.update_btn + '</button> <a class="delete">' + custom.delete_btn + '</a></span> \n';
					row += '</li> \n';

				$jq('#aec-category-table').append(row);
	...
	$jq('body').delegate('.update', 'click', function(e) {
		e.preventDefault();
		var row = $jq(this).parent().parent()[0],
			html_id = row.id,
			...
			if (cat.length > 1) {
				 $jq.post(ajaxurl, { action: 'update_category', 'category_data': json }, function(data, textStatus, jqXHR){
					if (data) {					
	...
	$jq('body').delegate('.delete', 'click', function(e) {
		e.preventDefault();
		var row = $jq(this).parent().parent()[0],
			html_id = row.id,
			id = html_id.replace('id_', ''),
				...
				if (data) {
					if (data == 'false') {
						if (confirm(custom.confirm_category_reassign)) {
							$jq.post(ajaxurl, { action: 'change_category', 'id': id }, function(data) {
